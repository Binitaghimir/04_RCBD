...
-col make accessding order of plot where col make desseceding order Now, let's plot it.

```{r crd map, paged.print=TRUE}
ggplot(data = crd_layout,
       aes(x = row, y= -col))+
  geom_tile(aes(fill = trtname), 
            color = "black",
            show.legend = F)+
  geom_text(aes(label = lab), vjust = 0.45)+
...
rcbd_rand %>% 
  ...
  dplyr::select(plot, rep, trtname,trtnum,
                nrate_kgha, krate_kgha) %>% write.csv("../Output/r2.csv")
