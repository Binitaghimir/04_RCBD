...
rcbd_cld_all <- cld(rcbd_means_all, 
                   ...
                   adjust= "",
               Letters=letters,
               alpha = 0.05) # by defualt is 0.05, use alpha if you want to change significant level
...
ggplot(mapping = aes(fill = trtname))+
  ...
  geom_jitter(data = rcbd_dfw, 
              aes(x = trtname, 
                  y = yield_kgha), 
              shape = 21, alpha = .6, size =3) + # to make transparent
geom_label(data = rcbd_cld_selected,
           aes(x = trtname,
               y = emmean, label = letter), fill = "white") + # use fill if you want to just chnage for geom_label otherwise it will take ggplot(aes)
  labs(x = "N rate + K rate(kg/ha)",
       y = "Yield (kg/ha")+
  scale_fill_viridis_d() +# if you want to customize the aes function use "scale"
#theme_classic()+
  theme(legend.position = "none", 
        panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank())
ggsave("../Output/rcbd.png",
       width = 8,
       height = 5)
```
